...
上面并行化的是**转换**这一步——计算每行输出的具体内容。写出这部分内容，如果
目标路径以 `.gz` 结尾，是另一个独立的问题，不特意处理的话会有自己的单线程
瓶颈——这是在 `fastx sample` 上踩坑之后才发现的（完整经过见
`docs/zh/sample.md` 里 `BlockWriter` 那一节——一份 57+59 GB 的真实数据集，
`-j` 实测完全不起作用，因为压缩不管开多少线程都只在一个线程上跑）。
`common::rename::run` 出于同样的原因迁移到了 `io_utils::BlockWriter`：每一批
转换完的行会按 `rayon::current_num_threads()` 切成若干组
（`lines_into_blocks`，见 `common/rename.rs`），并行压缩成各自独立的 gzip
member，而不是持续写进同一个长期存活的 `GzEncoder`。对明文输出来说这个改动
没有影响（没有压缩这一步可以并行）；但对大型参考基因组的 gzip 输出来说，
现在 `-j` 在写出这一侧也是真正在干活的，不只是转换那一侧。

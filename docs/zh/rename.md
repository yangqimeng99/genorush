...
2. 把这批行按线程数切成若干**连续**的组，每个 rayon 任务把自己那组转换后的
   内容直接追加进一个属于自己的字节缓冲区。`rayon` 的 `collect` 无论内部怎么
   把任务切给各个线程，最终都会保持和输入一致的顺序，所以这些缓冲区回来时
   就是有序的，不需要额外的重排步骤。
3. 这些缓冲区直接就是 `BlockWriter` 拿去并行压缩的"块"，重复直到文件读完。

第 2 步是和"显而易见的写法"分道扬镳的地方。那种写法是：把每一行转换成一个
`String`，收集起来，再拼接——每行分配一次，每行拷贝两次。写进属于任务自己的
缓冲区，则是**每组**才分配一次：不需要改的行就是一次 `memcpy`，需要改的行也
不过是几次追加。中间还试过返回 `Cow<str>`，这条路值得记下来作为反面教材——它
只能让"原样透传"的行省掉分配，所以在一份 seqid 全都在映射表里的 GFF 上（每行
都要重写），实测比它取代的 `String` 版本还**慢了约 11%**。缓冲区方案在两个方向
上都赢，因为它根本不关心这一行有没有变。

transform 的签名也由此而来：`Fn(&str, &HashMap<..>, &mut Vec<u8>)`，而不是返回
一行内容。是没那么好看，但这也是 `fastx/rename.rs` 和 `gff/rename.rs` 里那些
格式相关代码追加字节而不是拼字符串的原因。
...
下做了对比：所有测试场景输出都逐字节一致（`diff` 无差异）。这个对比不再是
一次性的了——`tests/rename_parity.rs` 会拿二进制的输出和该脚本产出的、提交在
`tests/data/rename/` 下的结果做比对，输入覆盖了上面契约里的每一条规则。

在一份 112MB、190 万行的合成 FASTA（29 条染色体）上，Python 脚本耗时约 5.9 秒，
本工具远低于 1 秒——这个差距主要来自 Python 的 `os.popen`/`less` 子进程开销和
解释器逐行处理的成本，而不是改名逻辑本身。

从"每行一个 `String`"改成"每任务一个缓冲区"（见上）的收益，在一份 311MB、
510 万行的 FASTA 和一份 319MB、500 万行的 GFF 上实测，两个版本交替运行 9 次取
最小值，每次都校验过输出逐字节一致：

| | 改造前 | 改造后 | |
|---|---|---|---|
| FASTA，明文 → 明文 | 0.77 s | 0.58 s | −24% |
| FASTA，明文 → gzip | 3.74 s | 3.38 s | −10% |
| FASTA，gzip → gzip | 4.24 s | 3.90 s | −8% |
| GFF，明文 → 明文 | 0.90 s | 0.57 s | −36% |
| GFF，每一行的 seqid 都要重写 | 0.89 s | 0.59 s | −33% |
| GFF，没有任何 seqid 在映射表里 | 0.86 s | 0.62 s | −27% |

明文 FASTA 这一场景的峰值内存也从 59 MB 降到 35 MB——中间那层 `Vec<String>`
没有了。gzip 那几行变化较小是因为压缩本身占了大头；反过来说，它们之所以还能
动，正是因为逐行处理这部分缩得足够多。
...
1. 如果转换逻辑是逐行、无状态的（就像这里一样），把它写成
   `Fn(&str, &HashMap<String, String>, &mut Vec<u8>)`——把结果追加进给定的
   缓冲区，而不是返回新的一行（如果共享状态不是名字映射表，可以进一步泛化
   `common::rename::run` 的签名），然后直接复用

...
genomics/transcriptomics data wrangling, written in Rust. Built in the
spirit of [seqkit](https://github.com/shenwei356/seqkit): a single static
binary, one command per task, no runtime dependencies.

Status: early stage, actively growing. Two commands exist today; more are
...
Every subcommand accepts a global `-j/--threads` flag (`0` = use all
logical cores).
...
- `src/common/` holds logic shared across categories: `rename.rs` (the
  ...
  record model), `rng.rs` (a dependency-free SplitMix64 RNG, both a
  stateless index-keyed variant for parallel sampling and a stateful
  variant for sequential algorithms like reservoir sampling).

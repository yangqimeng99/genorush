...
- `common::fastq::Mate` and `FastqRecord::mate_suffix()`: reads a `/1` or
  ...
  markers.
- Parity tests for `rename` (`tests/rename_parity.rs`, 14 cases). The
  commands claim byte-for-byte identical output to the Python script they
  replace, and that claim rested on a comparison done once by hand. Outputs
  generated by the original script are now committed under
  `tests/data/rename/`, over inputs that exercise every rule in the
  contract -- including the ones that look like bugs and are kept anyway: a
  dropped sequence description, a single-column GFF line coming out with a
  trailing tab, whitespace trimmed off both ends of every line.
...
- **`rename`'s per-line transforms write into a buffer instead of returning
  a string.** Each line used to become its own `String`, collected into a
  `Vec` and then copied again into the output blocks: one allocation and two
  copies per line. Each rayon task now appends its group of lines straight
  into the byte buffer that `BlockWriter` will compress, so an unchanged line
  is a memcpy and a rewritten one a few appends, with nothing allocated per
  line. Measured on a 311 MB FASTA and a 319 MB GFF, best of nine alternating
  runs with output verified byte-identical each time: plain FASTA 0.77 s ->
  0.58 s (-24%), plain GFF 0.90 s -> 0.57 s (-36%), and 8-10% off the gzip
  paths where compression dominates. Peak RSS on the plain FASTA case fell
  from 59 MB to 35 MB.

  A `Cow<str>` return type was tried first and rejected on measurement: it
  removes the allocation only for lines that pass through unchanged, so on a
  GFF whose seqids are all in the mapping -- every line rewritten, which is
  the ordinary case -- it came out ~11% slower than the `String` version it
  replaced. Recorded in `docs/*/rename.md` so the idea is not re-tried.
